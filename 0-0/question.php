...
<html lang="ja">
    ...
    <body>
        ...
        <form action = "answer.php" method = "POST">                                     <!--フォームの作成 通信はPOST通信で-->
            <h2>①ネットワークのポート番号は何番？</h2>
             <?php foreach ($question1 as $value) { ?>                                   <!--③ 問題のradioボタンを「foreach」を使って作成する-->
                <input type = "radio" name = "response1" value = "<?php echo $value ?>" > <?php echo $value ?>
                <?php } ?>

            <h2>②Webページを作成するための言語は？</h2>
                <?php foreach ($question2 as $value) { ?>                                <!--③ 問題のradioボタンを「foreach」を使って作成する-->   
                <input type = "radio" name = "response2" value = "<?php echo $value ?>" > <?php echo $value ?>
                <?php } ?>

            <h2>③MySQLで情報を取得するためのコマンドは？</h2>
                <?php foreach ($question3 as $value) { ?>                                 <!--③ 問題のradioボタンを「foreach」を使って作成する-->
                <input type = "radio" name = "response3" value = "<?php echo $value ?>" > <?php echo $value ?>
                <?php } ?>
                <!-- <input type = "hidden" name = "answer[]" value = "<?php echo $answer ?>" >    問題の正解の変数と名前の変数を[answer.php]に送る -->
